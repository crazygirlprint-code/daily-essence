...
export default function AIInsights() {
  ...
  const generateInsights = async () => {
    ...
    try {
      const [
        tasks, mealPlans, selfCareActivities, events,
        affirmations, meditationSessions, beautyRoutines, userProgress
      ] = await Promise.all([
        base44.entities.Task.list('-created_date', 50),
        base44.entities.MealPlan.list('-date', 20),
        base44.entities.SelfCareActivity.list('-created_date', 10),
        base44.entities.SpecialEvent.list('-date', 10),
        base44.entities.Affirmation.list(),
        base44.entities.MeditationSession.list('-completed_at', 10),
        base44.entities.BeautyRoutine.list(),
...
Be warm, specific, and actionable. Reference her actual data. Speak like a supportive friend who truly knows her routine.`;
  ...
  React.useEffect(() => {
    if (!hasAccess && authUser) {
      const accountAge = differenceInDays(new Date(), parseISO(authUser.created_date));
  ...
  // — no auto-generation to avoid API burst on page load
